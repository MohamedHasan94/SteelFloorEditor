...
class Editor {
    ...
    init() {
        ...
        this.scene.userData.beams = new THREE.Group();
        this.scene.add(this.scene.userData.beams);
    ...
    toggleBeams() {
        let beams = this.scene.userData.beams;
        let length = beams.children.length;
        let visual;
        for (let i = 0; i < length; i++) {
            visual = beams.children[i].userData.beam.visual;
            visual.temp = beams.children[i];

            visual.unusedMesh.position.copy(beams.children[i].position)
            visual.unusedMesh.rotation.copy(beams.children[i].rotation)

            beams.children[i] = visual.unusedMesh;
            visual.mesh = beams.children[i];
