...
function generateColumns(editor, coordX, coordY, coordZ, mainNodes, section) {
    ...
    for (let i = 0; i < xNo; i++) {
        ...
        for (let j = 0; j < zNo; j++) {
            ...
            let column = new Column(section, mainNodes[i * zNo + j].data.position.clone(), lowerNodes[i * zNo + j].data.position.clone(),
...
let hingeGeometry = new THREE.ConeBufferGeometry(0.2, 0.2, 4);
function createHinge(position) {
    let hinge = new THREE.Mesh(hingeGeometry, hingeMaterial.clone());
    hinge.position.set(position.x, -0.1 , position.z);
    return hinge
}
