...
function generateBeams(editor, coordX, coordZ, mainSection, secSection, secSpacing) {
    let mainBeams = [], secBeams = [], secCoord = [0], secNodes = [0], distribution;
    if (coordX[1] > coordZ[1]) {
        [mainBeams, nodes] = createZBeamsWithNodes(editor, coordX, coordZ, mainSection); //Create main beams on z-axis (short direction)
        ...
        [secBeams, secNodes] = createXBeams(editor, coordX, secCoord, secSection, coordZ, nodes, mainBeams);  //Create secondary beams on x-axis (long direction)
    }
    else {
        [mainBeams, nodes] = createXBeamsWithNodes(editor, coordX, coordZ, mainSection); //Create main beams on x-axis (short direction)

        distribution = coordX[coordX.length - 1]; // The distance over which sec beams are distributed
        for (let i = 1; secCoord[i - 1] < distribution; i++) { //Calculate the coordinates of sec beams
            secCoord[i] = secCoord[i - 1] + secSpacing;
        }
        [secBeams, secNodes] = createZBeams(editor, secCoord, coordZ, secSection, coordX, nodes, mainBeams);  //Create secondary beams on z-axis (long direction)
    }
...
function createXBeams(editor, coordX, coordZ, section, coordZToCheck, nodes, mainBeams) {
    let beams = [];
    let secBeamsNodes = [];
    let dimensions = new SectionDimensions(parseInt(section.split(' ')[1]) / 1000);
    let shape = createShape(dimensions);
...
function createZBeams(editor, coordX, coordZ, section, coordXToCheck, nodes, mainBeams) {
    let beams = [];
    let secBeamsNodes = [];
    let dimensions = new SectionDimensions(parseInt(section.split(' ')[1]) / 1000);
    let shape = createShape(dimensions);
...
function createZBeamsWithNodes(editor, coordX, coordZ, section) {
    let beams = [];
    let dimensions = new SectionDimensions(parseInt(section.split(' ')[1]) / 1000);
    let shape = createShape(dimensions);
...
function createXBeamsWithNodes(editor, coordX, coordZ, section) {
    let beams = [];
    let dimensions = new SectionDimensions(parseInt(section.split(' ')[1]) / 1000);
    let shape = createShape(dimensions);
